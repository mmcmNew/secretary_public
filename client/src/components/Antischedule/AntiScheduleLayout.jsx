...
import useAntiSchedule from "./hooks/useAntiSchedule";
...
export default function AntiScheduleLayout({ containerId }) {
  ...
  const sendUpdatedTaskToServer = updateAntiTask;
  ...
  function handleChangeEventStatus(eventId, updatedFields) {
    ...
    if (sendUpdatedTaskToServer && typeof sendUpdatedTaskToServer === "function")
      sendUpdatedTaskToServer({ taskId: eventId, ...updatedFields });
  }
  ...
  async function handleEventChange(eventInfo) {
    ...
    await sendUpdatedTaskToServer({ taskId: eventInfo.event.id, ...eventDict });
  }
    ...
    try {
        switch (currentTaskType) {
            case "event":
                ...
                await sendUpdatedTaskToServer({ taskId, ...updatedFields });
