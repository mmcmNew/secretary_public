...
def save_to_base_modules(target_module, command_type, message_info=None, files_list=None):
    ...
    # Если модуль относится к журналам, используем модель JournalEntry
    if module_cfg.get('type') == 'journal':
        match command_type:
            case 'create':
                entry = JournalEntry(user_id=current_user.id,
                                     journal_type=target_module,
                                     data=message_info)
                db.session.add(entry)
            case 'append':
                entry = JournalEntry.query.filter_by(user_id=current_user.id,
                                                     journal_type=target_module)
            ...
            case 'update':
                ...
                entry = JournalEntry.query.filter_by(id=record_id,
                                                     user_id=current_user.id,
                                                     journal_type=target_module).first()
                if not entry:
